    ...
    rows: [
      {
        key: 'vastdata',
        name: 'VastData',
        pros: '支持多种存储协议，可作为 Ceph 的替代方案；支持多租户、QoS 与去重；规模扩大后可摊薄授权成本；提供原厂技术支持',
        cons: '性能略低于 GPFS ECE，采购周期较长',
        scenarios: '需要 QoS 和技术支持的多租户场景',
      },
      {
        key: 'gpfs-ece',
        name: 'GPFS ECE',
        pros: '性能高、生态成熟，软件授权费用低',
        cons: '多租户支持有限，依赖第三方厂商技术支持',
        scenarios: '预算有限、需要高性能的单租户场景',
      },
      {
        key: 'gpfs-hybrid',
        name: 'GPFS 混闪',
        pros: '单位容量成本远低于全闪方案；大块读写带宽随 HDD 数量线性增长；与全闪 GPFS 共用运维体系',
        cons: '小文件随机读写性能依赖 NVMe 层命中率；HDD 重建耗时较长；多租户支持有限',
        scenarios: '冷、温数据量大，且以大文件顺序读写为主的场景',
      },
      {
        key: 'weka',
        name: 'Weka',
        ...
        scenarios: '预算充足、追求极致性能的场景',
      },
      {
        key: 'ceph',
        name: 'CephFS',
        pros: '开源、无软件授权费用，支持多租户',
        cons: '不支持 QoS；元数据缓存受节点内存限制，内存不足时性能显著下降；运维成本高；无技术支持',
        scenarios: '预算有限、非 AI 场景的通用共享文件存储',
      },
    ],
    notes: ['不建议将 CephFS 用于 AI 场景'],
  },
  {
    title: '对象存储',
    rows: [
      {
        key: 'xeos',
        name: 'XSKY XEOS',
        pros: '功能完善、运行稳定，支持大规模扩展与 QoS，并提供原厂技术支持',
        cons: '软件授权费用高',
        scenarios: '重视稳定性和技术支持的生产环境',
      },
      {
        key: 'ceph-hybrid',
        name: 'Ceph RGW',
      ...
      },
      {
        key: 'vastdata',
        name: 'VastData S3',
        pros: '性能高，可与文件系统共用集群，支持 QoS 与大规模扩展，并提供原厂技术支持',
...
export const STORAGE_INFO: Record<StorageKey, { description: string; pros: string[]; cons: string[]; limits?: string[] }> = {
  xeos: {
    ...
    cons: ['软件授权费用较高', '得盘率相对较低，整体成本偏高'],
  },
  vastdata: {
    description: 'VastData 是全闪统一存储平台，单一系统同时提供文件、对象和块存储服务，基于 NVMe SSD 和 SCM 构建。',
    pros: ['支持多种存储协议，可作为 Ceph 的替代方案', '支持多租户', '去重与压缩提升集群可用容量', '支持 QoS（含元数据 QoS）', '原厂技术支持'],
    cons: ['采购费用高于 GPFS', '采用大容量 QLC SSD，性能低于使用小容量 TLC SSD 的 GPFS 等方案', '采购周期较长'],
  },
  'gpfs-ece': {
    description: 'IBM GPFS/Scale ECE（Erasure Coding Edition）是高性能并行文件系统，基于 NVMe SSD 和 RDMA 网络构建。',
    pros: ['性能高', '采购成本低'],
    cons: ['多租户支持有限', '运维成本高', '原厂支持有限'],
    limits: ['启用多租户时，初始容量与每次扩容的步长均为 50 TiB', '启用多租户时，Kubernetes 仅支持 hostPath，不支持基于 CSI 的 PVC'],
  },
  'gpfs-hybrid': {
    description: 'GPFS/Scale 混闪采用大容量 HDD 搭配少量 NVMe SSD：NVMe 层存储元数据和热数据，HDD 层存储冷数据，以远低于全闪方案的成本提供大容量并行文件存储。',
    pros: [
      '每 TB 成本远低于全闪方案',
      '开启分层后热数据命中 NVMe，带宽约为纯 HDD 的 1.5–2 倍，IOPS 可达 8 倍以上',
      '大块顺序读写带宽随 HDD 数量线性增长',
      '与全闪 GPFS 共用软件和运维体系，可通过混合组池实现分层',
    ],
    cons: [
      '性能高度依赖 NVMe 层命中率，命中率低时回落到 HDD 水平（IOPS 差距接近一个数量级）',
      'HDD 重建时间长，重建期间性能下降明显',
      '多租户支持有限，运维成本高，原厂支持有限',
    ],
    limits: [
  ...
  ceph: {
    ...
    cons: [
      '不支持折叠纠删码，初始节点数较少时，得盘率较低',
      '磁盘间的数据分布不均会进一步减少可用容量，本工具按 70% 的均衡系数计算',
      '全闪性能一般，高性能需求下需增加磁盘数量',
      '文件系统元数据缓存受节点内存限制，内存不足时性能显著下降',
      '文件系统元数据需额外配置多个大内存节点',
      '文件系统运维成本高',
  ...
  'ceph-hybrid': {
    description: 'Ceph 混闪配置基于大量 HDD 和少量 NVMe SSD 构建，混闪架构仅建议用于 Ceph RGW 对象存储，适合以较低成本存储海量非结构化数据。',
    pros: [
      '开源软件，无需购买软件授权',
      '支持多租户',
      '大容量 HDD 硬件成本低',
  ...
  weka: {
    ...
    pros: ['性能出色，在所列方案中最高', '支持分层到对象存储，构建低成本混闪文件系统', '支持多租户', '支持 QoS'],
    cons: ['软件授权费用较高', '依赖第三方厂商提供技术支持'],

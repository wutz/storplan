...
存储容量与性能规划工具。输入容量和带宽，对比多种存储方案的集群规模、硬件配置和性能。
...
- **VastData** — 全闪统一存储（NFS、SMB、S3、iSCSI、NVMe-oF）
- **GPFS ECE** — 全闪并行文件系统
...
右下角的「AI 规划助手」支持多轮对话：用自然语言说说业务需求（数据量、协议、GPU 规模、预算），
模型问清关键条件后选出方案、定出容量与带宽，参数自动填进页面顶部的规划表单，由本站已有的计算逻辑出结果——
模型只负责选型和定参，集群规模和性能数字仍由本地算法计算。

**先问清，再规划。** 规划结果是能拿去询价的配置，前提错了，算出来的配置比不算更糟，所以助手默认先问清需求：
...
另外：话题限定在存储、Kubernetes、网络、GPU、AI 基础设施，其他问题会被拒答；需要厂商参数等站外信息时会联网检索；
填进表单的参数随时可以手改。

**对话框操作**：拖动卡头可以移动，拖左上角手柄可以缩放（右下角固定，双击卡头回到默认位置），位置和尺寸记在
localStorage 里。点面板外侧或按 Esc 收起。开合是 macOS「缩放特效」那种观感：窗口从右下角那颗按钮里长出来、收起时缩回按钮
（transform-origin 锚在按钮中心，面板被拖到哪儿都朝它飞），`prefers-reduced-motion` 下直接切换。
收起时面板不卸载，所以再打开时消息与滚动位置都停在离开前。
每张方案卡记着自己那一组参数：表单后来被改过时，按钮会变成「恢复这组参数并查看」，点一下即可还原再看结果。
...
未配置 `LLM_API_KEY` 时，助手会提示还没配置，页面其余功能不受影响。
...
**仍然存在的两个缺口**，别把 WAF 当成花费上限：

1. **不覆盖预览域**：WAF 只对自有域生效，`*.workers.dev` 绕过它，只剩绑定那层兜底。预览域是给 PR
   验证用的、不对外公布；要彻底堵死就在生产切到自有域后关掉 `workers_dev`。
2. **限的是速率，不是总量**：`wutz.dev` 是 Free 套餐，窗口固定 10 秒，最紧也只能压到 1–2 次/10s，
   换算下来单 IP 每天仍可发起上万次请求，而每次都是一次 Opus 调用。WAF 大幅拉高了刷量成本，
   但真要给花费设硬顶，得再上其中一条：上游 API Key 侧的额度限制、Durable Object 做每日配额、
   或给助手加访问控制（Turnstile / 登录）。
...
- [Storpath](https://storpath.wutz.dev/) —— 存储运维工程师成长路径，交互式课程（[源码](https://github.com/wutz/storpath)）。这边算出的容量和性能口径，在那边的 L3「容量与性能规划」里有推导过程。

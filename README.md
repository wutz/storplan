...
**对话框操作**：拖卡头可移动、拖左上角手柄可缩放（右下角固定，双击卡头恢复默认位置），位置与尺寸记在
localStorage 里。点面板外或按 Esc 收起。开合是 macOS「缩放特效」那种观感：窗口从右下角那颗按钮里长出来、收起时缩回按钮
（transform-origin 锚在按钮中心，面板被拖到哪儿都朝它飞），`prefers-reduced-motion` 下直接切换。
收起时面板不卸载，所以再打开时消息与滚动位置都停在离开前。
每张方案卡记着自己那一组参数：表单后来被改过时按钮变成「恢复这组参数并查看」，点一下还原再看结果。
